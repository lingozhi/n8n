...
FROM node:24.15.0-alpine3.22 AS chinese-editor-ui

ARG N8N_I18N_CHINESE_VERSION
ARG N8N_I18N_CHINESE_SHA256
...
RUN set -eux; \
	...
	mkdir -p /tmp/editor-ui; \
	tar -xzf "$archive" -C /tmp/editor-ui --strip-components=1; \
	rm -f "$archive"
...
COPY --from=chinese-editor-ui --chown=node:node /tmp/editor-ui/ /usr/local/lib/node_modules/n8n/node_modules/n8n-editor-ui/dist/
